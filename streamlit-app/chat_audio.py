...
def transcribe_audio(audio_bytes):
    ...
    Requires OPENAI_API_KEY in st.secrets or env vars.
    """
    try:
        # OpenAI client looks for OPENAI_API_KEY in env by default
        # Or you can pass it explicitly: client = OpenAI(api_key=st.secrets["OPENAI_API_KEY"])
        client = OpenAI()
...
def process_message(user_text, model, cache_dir):
    """
    Refactored logic to handle a user message (from Text OR Audio).
    """
    current_chat = get_current_chat()

    # 1. Append user message
    current_chat["messages"].append({"role": "user", "content": user_text})
    update_chat_title_from_prompt(current_chat, user_text)

    # 2. Display user message immediately (if not already displayed by logic)
    ...
    with st.chat_message("assistant"):
        ...
        if exit_code != 0 or answer is None:
            ...
            if isinstance(answer, dict):
                content = str(answer.get("answer", answer))
                # formatting for code blocks if needed
            ...
            st.markdown(content)
            current_chat["messages"].append({"role": "assistant", "content": content})
...
with st.sidebar:
    ...
    st.caption("Record to auto-transcribe & send.")

    # The recorder returns a dict: {'bytes': b'...', 'sample_rate': ...}
    audio_data = mic_recorder(
        start_prompt="Record Audio (Whisper)",
        stop_prompt="Stop & Send",
        just_once=True,
        key="voice_recorder"  # Important for state
    )
...
# Check if Audio was just recorded
if audio_data is not None:
    ...
    if transcribed_text:
        ...
        st.toast(f"🗣️ Heard: {transcribed_text}")

        if not model.strip():
            st.warning("Please specify a model in the sidebar.")
        else:
            # Process using the shared function
            process_message(transcribed_text, model, cache_dir)

            # Force a rerun to clear the recorder state and update history visually
            st.rerun()
...
if prompt:
    ...
    # Process using the shared function
    process_message(prompt, model, cache_dir)

    # Rerun to redraw conversation
    st.rerun()
